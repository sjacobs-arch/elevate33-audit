...
// ─── file reading ────────────────────────────────────────────────────────────
// Vendor Central uses "14 Day Total Sales/Orders", Seller uses "7 Day" — normalize at parse time
const VENDOR_ALIAS = {
  "14 Day Total Sales":       "7 Day Total Sales",
  "14 Day Total Orders":      "7 Day Total Orders",
  "14 Day Total Orders (#)":  "7 Day Total Orders (#)",
};
...
const normCols = (data) =>
  data.map((r) => {
    ...
    for (const k of Object.keys(r)) {
      ...
      nr[VENDOR_ALIAS[trimmed] || trimmed] = r[k];
    }
    return nr;
  });
...
const LeadGate = ({ onUnlock }) => {
  ...
  const handleSubmit = async () => {
    if (!valid) return;
    setStatus("loading");
    try { await submitToApollo(form); } catch (e) { console.warn("Apollo error:", e.message); }
    onUnlock();
  };

  const inputStyle = {
    width: "100%", boxSizing: "border-box", padding: "11px 14px", borderRadius: 10,
    fontSize: 14, border: "1.5px solid #e2e8f0", outline: "none", fontFamily: "inherit",
    transition: "border-color 0.2s", background: "white", color: "#0f172a",
  };
  const labelStyle = {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 999, background: "linear-gradient(150deg,#f0f3ff 0%,#f8f9fb 50%,#fff8f3 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ background: "white", borderRadius: 24, padding: "44px 44px 40px", maxWidth: 480, width: "100%", position: "relative", boxShadow: "0 24px 80px rgba(15,23,42,0.12), 0 4px 16px rgba(79,70,229,0.08)", border: "1px solid rgba(79,70,229,0.1)" }}>
        ...
        <h1 style={{ margin: "0 0 8px", fontSize: 22, fontWeight: 900, color: "#0f172a", textAlign: "center", letterSpacing: "-0.03em" }}>Amazon Ads Audit Tool</h1>
        <p style={{ margin: "0 0 28px", fontSize: 14, color: "#64748b", textAlign: "center", lineHeight: 1.6 }}>Free, instant account analysis. Enter your info to get started.</p>

        <div style={{ display: "flex", alignItems: "flex-start", gap: 10, background: "linear-gradient(135deg,rgba(79,70,229,0.05),rgba(249,115,22,0.04))", border: "1px solid rgba(79,70,229,0.12)", borderRadius: 12, padding: "12px 14px", marginBottom: 24 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#64748b", lineHeight: 1.5 }}>All report processing happens locally on your device. Your Amazon data is never uploaded to any server.</div>
          </div>
        </div>
        ...
        <button onClick={handleSubmit} disabled={!valid || status === "loading"}
          style={{ width: "100%", padding: "14px", borderRadius: 12, border: "none", cursor: valid ? "pointer" : "not-allowed", background: valid ? `linear-gradient(135deg,${P.purple2},${P.orange3})` : "#e2e8f0", color: valid ? "white" : "#94a3b8", fontSize: 15, fontWeight: 800, letterSpacing: "-0.01em", fontFamily: "inherit", transition: "opacity 0.2s", boxShadow: valid ? `0 4px 20px ${P.purple2}40` : "none" }}
...
const PAGES = [
  ...
  { id: "s1",     label: "Brand"       },
  { id: "s2",     label: "Match Type"  },
  { id: "s3",     label: "Placements"  },
  { id: "s4",     label: "ASINs"       },
  { id: "report", label: "Report"      },
];

// ─── progress bar ─────────────────────────────────────────────────────────────
const ProgressBar = ({ current, onNav, results }) => {
  const ci = PAGES.findIndex(p => p.id === current);
  return (
    <div style={{ background: "white", borderBottom: "1px solid #eaedf5", padding: "0 28px", height: 44, display: "flex", alignItems: "center", gap: 0, overflowX: "auto" }}>
      {PAGES.map((p, i) => {
        const active   = p.id === current;
        const done     = i < ci;
        const locked   = !results && i > 1;
        const canClick = !locked && (done || i <= ci + 1);
        return (
          <div key={p.id} style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
            {i > 0 && <div style={{ width: 24, height: 1, background: done ? P.purple2 : "#e2e8f0", transition: "background 0.3s" }} />}
            <button onClick={() => canClick && onNav(p.id)} style={{
              display: "flex", alignItems: "center", gap: 6, padding: "4px 10px",
              borderRadius: 20, border: "none", cursor: canClick ? "pointer" : "default",
              background: active ? `linear-gradient(135deg,${P.purple2},${P.purple3})` : "transparent",
              color: active ? "white" : done ? P.purple2 : "#94a3b8",
              fontSize: 11, fontWeight: active ? 700 : 600, fontFamily: "inherit",
              transition: "all 0.2s",
            }}>
              <div style={{
                width: 16, height: 16, borderRadius: "50%", flexShrink: 0,
                background: active ? "rgba(255,255,255,0.3)" : done ? P.purple2 : "#e2e8f0",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 9, fontWeight: 800, color: active ? "white" : done ? "white" : "#94a3b8",
...
const NavFooter = ({ current, onNav, canAdvance = true, advanceLabel = "Next →" }) => {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 32, paddingTop: 24, borderTop: "1px solid #f1f5f9" }}>
      <div>
        {prev && (
          <button onClick={() => onNav(prev)} style={{
      ...
      <div>
        {next && (
          <button onClick={() => canAdvance && onNav(next)} disabled={!canAdvance} style={{
            padding: "10px 24px", borderRadius: 10, border: "none",
            background: canAdvance ? `linear-gradient(135deg,${P.purple2},${P.orange3})` : "#e2e8f0",
            color: canAdvance ? "white" : "#94a3b8", fontSize: 13, fontWeight: 700,
            cursor: canAdvance ? "pointer" : "not-allowed", fontFamily: "inherit",
            boxShadow: canAdvance ? `0 4px 14px ${P.purple2}40` : "none",
            transition: "all 0.2s",
          }}>{advanceLabel}</button>
        )}
      </div>
...
const IntroPage = ({ onStart }) => (
  <div style={{ maxWidth: 780, margin: "0 auto", padding: "60px 20px" }}>
    ...
    <div style={{ textAlign: "center", marginBottom: 56 }}>
      <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: `linear-gradient(135deg,${P.purple2}15,${P.orange3}10)`, border: `1px solid ${P.purple2}25`, borderRadius: 20, padding: "5px 14px", marginBottom: 24 }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: P.purple2, textTransform: "uppercase", letterSpacing: "0.08em" }}>Free Tool by ELEVATE33</span>
      </div>
      <h1 style={{ margin: "0 0 16px", fontSize: 40, fontWeight: 900, color: "#0f172a", letterSpacing: "-0.04em", lineHeight: 1.15 }}>
        Amazon Sponsored Products<br />
        <span style={{ background: `linear-gradient(135deg,${P.purple2},${P.orange3})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Audit Tool</span>
      </h1>
      <p style={{ margin: "0 0 36px", fontSize: 16, color: "#475569", lineHeight: 1.7, maxWidth: 560, marginLeft: "auto", marginRight: "auto" }}>
        Upload your Seller Central and Amazon Ads reports for an instant, AI-powered breakdown of your brand strategy, match type efficiency, placement performance, and ASIN-level revenue.
      </p>
      <button onClick={onStart} style={{
        padding: "14px 36px", borderRadius: 12, border: "none",
        background: `linear-gradient(135deg,${P.purple2},${P.orange3})`,
        color: "white", fontSize: 16, fontWeight: 800, cursor: "pointer",
        fontFamily: "inherit", letterSpacing: "-0.01em",
        boxShadow: `0 6px 24px ${P.purple2}45`,
      }}>Start Your Audit →</button>
    </div>

    {/* What you get */}
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 16, marginBottom: 48 }}>
      {[
        { icon: "🏷", title: "Brand vs. Non-Brand", desc: "See exactly how much of your spend is defending your brand vs. acquiring new customers." },
        { icon: "🎯", title: "Match Type Analysis", desc: "Identify wasted spend by match type and find your highest-converting search term buckets." },
        { icon: "📍", title: "Placement Performance", desc: "Compare ROAS and CVR across Top of Search, Rest of Search, and Product Pages." },
        { icon: "📦", title: "ASIN Revenue Breakdown", desc: "Merge ad data with organic sales to see true revenue contribution per product." },
      ].map(({ icon, title, desc }) => (
        <div key={title} style={{ background: "white", borderRadius: 16, padding: "22px 24px", border: "1px solid #e8ecf0", boxShadow: "0 2px 8px rgba(15,23,42,0.04)" }}>
    ...
    {/* Reports needed */}
    <div style={{ background: "white", borderRadius: 16, padding: "24px 28px", border: "1px solid #e8ecf0" }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>Reports You'll Need</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 10 }}>
        {FILE_CFG.map((c) => (
          <div key={c.id} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
            <div style={{ width: 20, height: 20, borderRadius: 6, background: `linear-gradient(135deg,${P.purple2},${P.purple4})`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
              <span style={{ color: "white", fontSize: 10, fontWeight: 800 }}>↓</span>
            </div>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#1e293b" }}>{c.label}</div>
              <div style={{ fontSize: 11, color: "#94a3b8" }}>{c.hint}</div>
            </div>
          </div>
        ))}
      </div>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "center", marginTop: 28 }}>
      ...
      <span style={{ fontSize: 12, color: "#94a3b8" }}>All data is processed locally in your browser — nothing is ever uploaded to a server.</span>
    </div>
  </div>
);

// ─── report page ──────────────────────────────────────────────────────────────
const ReportPage = ({ results, onNav }) => {
  const [insights, setInsights] = useState({ s1: "", s2: "", s3: "", s4: "" });
  const [loading,  setLoading]  = useState(false);
  const [done,     setDone]     = useState(false);

  const sections = [
    { key: "s1", label: "Brand vs. Non-Brand",   icon: "🏷", payload: buildS1Payload(results.s1), accent: P.purple2 },
    { key: "s2", label: "Match Type Analysis",    icon: "🎯", payload: buildS2Payload(results.s2), accent: P.purple3 },
    { key: "s3", label: "Placement Performance",  icon: "📍", payload: buildS3Payload(results.s3), accent: P.purple4 },
    { key: "s4", label: "ASIN Revenue",           icon: "📦", payload: buildS4Payload(results.s4), accent: P.orange2 },
  ];
  ...
  const generateAll = async () => {
    ...
    await Promise.all(sections.map(async (s) => {
      try {
        results_map[s.key] = await callClaude(s.payload);
      } catch (e) {
        results_map[s.key] = "Unable to generate insight for this section.";
      }
    }));
    setInsights(results_map);
    setLoading(false);
    setDone(true);
  };

  const s1k = results.s1.kpis;
  const s4d = results.s4;
  const totalAdSales = s4d.data.reduce((s, d) => s + d.adSales, 0);

  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 20px 80px" }}>
      {/* Header */}
      <div style={{ textAlign: "center", marginBottom: 40 }}>
        <h1 style={{ margin: "0 0 8px", fontSize: 28, fontWeight: 900, color: "#0f172a", letterSpacing: "-0.03em" }}>
          Your Audit Report
        </h1>
        <p style={{ margin: 0, fontSize: 14, color: "#64748b" }}>AI-powered strategic summary across all four analysis sections</p>
      </div>

      {/* Top KPI summary strip */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 32 }}>
        {[
          { label: "Total Ad Spend",  value: f.$(s1k.totalSpend) },
          { label: "Total Ad Sales",  value: f.$(s1k.totalSales) },
          { label: "Overall ROAS",    value: f.x(s1k.overallROAS) },
          { label: "Brand Spend %",   value: f.pct(s1k.brandPct) },
        ].map(({ label, value }) => (
          <div key={label} style={{ background: "white", borderRadius: 14, padding: "16px 18px", border: "1px solid #e8ecf0", textAlign: "center" }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>{label}</div>
            <div style={{ fontSize: 20, fontWeight: 900, color: "#0f172a", letterSpacing: "-0.02em", fontFamily: '"DM Mono", monospace' }}>{value}</div>
          </div>
        ))}
      </div>
      ...
      {!done && (
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <button onClick={generateAll} disabled={loading} style={{
            padding: "14px 36px", borderRadius: 12, border: "none",
            background: loading ? "#e2e8f0" : `linear-gradient(135deg,${P.purple2},${P.orange3})`,
            color: loading ? "#94a3b8" : "white", fontSize: 15, fontWeight: 800,
            cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit",
            boxShadow: loading ? "none" : `0 6px 24px ${P.purple2}40`,
          }}>
            {loading ? (
              <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ display: "inline-block", width: 16, height: 16, borderRadius: "50%", border: "2.5px solid white", borderTopColor: "transparent", animation: "spin 0.7s linear infinite" }} />
                Generating all insights…
              </span>
            ) : "✦ Generate Full AI Report"}
          </button>
          <p style={{ margin: "10px 0 0", fontSize: 12, color: "#94a3b8" }}>Analyzes all 4 sections simultaneously — takes about 10 seconds</p>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        {sections.map((s) => (
          <div key={s.key} style={{
            ...
            border: "1px solid #e8ecf0", boxShadow: "0 2px 8px rgba(15,23,42,0.05)",
            position: "relative", overflow: "hidden",
          }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg,${s.accent},${P.orange3})` }} />
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              <span style={{ fontSize: 22 }}>{s.icon}</span>
              <span style={{ fontSize: 15, fontWeight: 800, color: "#0f172a" }}>{s.label}</span>
            </div>
            {!done && !loading && (
              <div style={{ fontSize: 13, color: "#94a3b8", fontStyle: "italic" }}>Click "Generate Full AI Report" above to populate insights.</div>
            )}
            {loading && (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 14, height: 14, borderRadius: "50%", border: `2px solid ${s.accent}`, borderTopColor: "transparent", animation: "spin 0.7s linear infinite" }} />
                <span style={{ fontSize: 13, color: "#64748b", fontStyle: "italic" }}>Analyzing…</span>
              </div>
            )}
            {done && insights[s.key] && (
              <p style={{ margin: 0, fontSize: 14, color: "#1e293b", lineHeight: 1.8 }}>{insights[s.key]}</p>
            )}
          </div>
        ))}
      </div>
      ...
      {done && (
        <div style={{ marginTop: 28, padding: "18px 22px", background: `linear-gradient(135deg,${P.purple2}08,${P.orange3}06)`, border: `1px solid ${P.purple2}20`, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div style={{ fontSize: 13, color: "#475569", fontWeight: 600 }}>Want a hands-on audit from the ELEVATE33 team?</div>
          <a href="https://elevate33.com" target="_blank" rel="noopener noreferrer" style={{
      ...
      <NavFooter current="report" onNav={onNav} />
    </div>
  );
};
        ...
        <header style={{ background: "rgba(255,255,255,0.93)", backdropFilter: "blur(20px)", borderBottom: "1px solid #eaedf5", padding: "0 28px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 1px 0 rgba(0,0,0,0.04)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ display: "flex", gap: 4 }}>
              {FILE_CFG.map((c) => (
                <div key={c.id} style={{ width: 6, height: 6, borderRadius: "50%", background: parsedData[c.id]?.length ? P.orange3 : "#e2e8f0", transition: "background 0.3s" }} />
              ))}
            </div>
            <span style={{ fontSize: 11, color: "#64748b", fontWeight: 600, fontFamily: '"DM Mono", monospace' }}>{loadedCount}/5</span>
          </div>
        </header>
        ...
        <main style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 20px" }}>
              ...
              <div style={{ marginBottom: 28 }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 4 }}>
                  <h1 style={{ margin: 0, fontSize: 22, fontWeight: 900, color: "#0f172a", letterSpacing: "-0.03em" }}>Upload Your Reports</h1>
                  {allLoaded && (
                    <span style={{ fontSize: 12, fontWeight: 700, color: P.orange3, background: "rgba(249,115,22,0.1)", padding: "2px 10px", borderRadius: 20 }}>
                      All reports loaded ✓
                    </span>
                  )}
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 10 }}>
                  {FILE_CFG.map((cfg) => (
                    <UploadCard key={cfg.id} cfg={cfg}
                      loaded={!!parsedData[cfg.id]?.length}
                      loading={loadingId === cfg.id}
                      fileName={fileNames[cfg.id] || ""}
                      onFile={handleFile}
                    />
                  ))}
                </div>
              </div>
              <NavFooter current="upload" onNav={nav} canAdvance={allLoaded && !!results} advanceLabel="Run Audit →" />
            </div>
          )}
          ...
          {page === "s1" && results && (
            <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 20px" }}>
              ...
              <NavFooter current="s1" onNav={nav} />
            </div>
          )}
          ...
          {page === "s2" && results && (
            <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 20px" }}>
              ...
              <NavFooter current="s2" onNav={nav} />
            </div>
          )}
          ...
          {page === "s3" && results && (
            <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 20px" }}>
              <SectionCard num="3" title="Placement Performance" subtitle="Spend and ROAS/CVR across ad placements" accent={P.purple4}>
                <ResponsiveContainer width="100%" height={340}>
                  <ComposedChart data={results.s3.chart} margin={{ top: 16, right: 80, bottom: 24, left: 20 }}>
                    <defs>
                    ...
                    <Line yAxisId="right"  type="monotone" dataKey="roas" name="ROAS" stroke={P.orange3} strokeWidth={2.5}
                      dot={{ fill: P.orange3, r: 6, strokeWidth: 2, stroke: "white" }}
                      activeDot={{ r: 8, stroke: P.orange3, strokeWidth: 2 }} />
                    <Line yAxisId="right2" type="monotone" dataKey="cvr"  name="CVR"  stroke={P.purple4} strokeWidth={2.5} strokeDasharray="5 3"
                      dot={{ fill: P.purple4, r: 6, strokeWidth: 2, stroke: "white" }}
                      activeDot={{ r: 8, stroke: P.purple4, strokeWidth: 2 }} />
                  </ComposedChart>
                </ResponsiveContainer>
                <InsightCard sectionKey="s3" payload={buildS3Payload(results.s3)} />
              </SectionCard>
              <NavFooter current="s3" onNav={nav} />
            </div>
          )}
          ...
          {page === "s4" && results && (
            <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 20px" }}>
              <SectionCard num="4" title="ASIN Budget & Revenue Relationship" subtitle="Organic sales (Business Report) merged with advertising investment by product ASIN" accent={P.purple2}>
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <p style={{ margin: "0 0 12px", fontSize: 12, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                        Top 10 ASINs — Revenue vs. Ad Spend
                      </p>
                      <ResponsiveContainer width="100%" height={300}>
                        <ComposedChart data={top10} margin={{ top: 8, right: 70, bottom: 40, left: 20 }}>
                          ...
                          <Line yAxisId="right" type="monotone" dataKey="Ad Spend" name="Ad Spend" stroke={P.orange3} strokeWidth={2.5}
                            dot={{ fill: P.orange3, r: 5, strokeWidth: 2, stroke: "white" }}
                            activeDot={{ r: 8, stroke: P.orange3, strokeWidth: 2 }} />
                        </ComposedChart>
                      </ResponsiveContainer>
              ...
              <NavFooter current="s4" onNav={nav} advanceLabel="View Report →" />
            </div>
          )}

          {/* ── REPORT ── */}
          {page === "report" && results && <ReportPage results={results} onNav={nav} />}
